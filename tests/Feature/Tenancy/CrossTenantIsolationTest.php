...
it('isolates tenant data by physical database (Eloquent and raw query)', function () use (&$provisioned): void {
    ...
    // --- Write a row inside tenant A's context (tenant-owned Work data) ---
    ...
    $rawA = DB::select('select count(*) as c from projects where name = ?', ['A-secret']);
    ...
    // READ — Eloquent: B's projects table is empty; A's id is unreachable.
    expect(Project::query()->count())->toBe(0)
        ->and(Project::find($projectId))->toBeNull();

    // READ — raw query: zero rows for A's data on B's connection.
    $rawB = DB::select('select count(*) as c from projects where name = ?', ['A-secret']);
    expect((int) $rawB[0]->c)->toBe(0);

    // UPDATE — Eloquent + raw affect zero rows (nothing to touch in B's DB).
    expect(Project::query()->where('name', 'A-secret')->update(['description' => 'pwned']))->toBe(0)
        ->and(DB::update('update projects set description = ? where name = ?', ['pwned', 'A-secret']))->toBe(0);

    // DELETE — Eloquent + raw affect zero rows.
    expect(Project::query()->where('name', 'A-secret')->delete())->toBe(0)
        ->and(DB::delete('delete from projects where name = ?', ['A-secret']))->toBe(0);
...
it('restores tenant context after a queued job runs (no context bleed)', function () use (&$provisioned): void {
    ...
    // Write one project in A, two in B, each inside its own tenant context. A
    // job dispatched in a tenant's context re-initializes THAT tenant on
    // execution (multitenancy §3). With QUEUE_CONNECTION=sync the closure runs
    // inline but still through the tenancy-aware dispatch path.
    $tenantA->run(function (): void {
        Project::create(['name' => 'a1', 'status' => ProjectStatus::Planning]);
    });

    $tenantB->run(function (): void {
        Project::create(['name' => 'b1', 'status' => ProjectStatus::Planning]);
        Project::create(['name' => 'b2', 'status' => ProjectStatus::Planning]);
    });

    // Each tenant sees ONLY its own rows — context did not bleed across runs.
    expect($tenantA->run(fn (): int => Project::query()->count()))->toBe(1)
        ->and($tenantB->run(fn (): int => Project::query()->count()))->toBe(2);
